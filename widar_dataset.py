...
def preprocess_csi_gen(csi, noise_sigma, output_len):
    process_csi = csi/noise_sigma

    process_csi = fit_data_size(process_csi, target_size=2048)
    process_csi = interpolate_data(process_csi, target_size=output_len)
...
def _preprocess_worker(args):
    file_path, output_len, min_data_len = args
    data = np.load(file_path)
    csi = np.array(data['csi_data'])
    if csi.shape[0] < min_data_len:
        return file_path, None
    noise_sigma = np.array(data['noise_array'])
    processed_csi = preprocess_csi_gen(csi, noise_sigma, output_len)
    return file_path, processed_csi

class WiDARDataset(Dataset):
    def __init__(self, dir_path, output_len, min_data_len=1024, transform=None, split_ratio=0.8, split_seed=42, must_have=None, must_not_have=None):
        ...
        temp_results = {}
        worker_args = [(fp, self.output_len, self.min_data_len) for fp in self.file_paths]
        ...
        self.preprocessed_data = torch.empty((len(self.file_paths), self.output_len, num_features), dtype=torch.float32)
        for fp, p_csi in temp_results.items():
            idx = self.path_to_idx[fp]
            self.preprocessed_data[idx] = torch.tensor(p_csi, dtype=torch.float32)
...
if __name__ == "__main__":
    ...
    dataset = WiDARDataset('widar_data', output_len=512, must_have=must_have)
    len_list = []
    more_than_1536 = 0
    less_than_1024 = 0
